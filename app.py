...
transmission_server = "https://" + config.get("transmission", "host") + "/ui/rpc"
...
def download_missing():
    ...
    db.query("SELECT s.show_id, season, number, path, MIN(magnet) magnet, s.name show_name FROM episodes e JOIN shows s ON e.show_id = s.show_id  WHERE `download` = 1 GROUP BY s.show_id, season, number, s.name, path HAVING MAX(downloaded) = 0")
    res = db.store_result()
    i = res.fetch_row(how=1)
